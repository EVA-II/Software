"""Input parsing, feature preparation, and threshold-based warning rules for bridge assessment."""
...
class BridgePreprocessor:
    ...
    def _engineered_dataframe_to_graph(
        ...
        if all(column in dataframe.columns for column in settings.target_column_names):
            targets_raw = dataframe[settings.target_column_names].astype(float)
            try:
        ...
        try:
            scaled_features = self.scaler_x.transform(feature_frame)
        except Exception as exc:
